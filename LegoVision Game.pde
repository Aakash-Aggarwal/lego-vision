...
void draw() {
  ...
  for (int i = 0; i < mPhysics.particles().size(); i++) {
    ...
    if(mParticle.position().y > height){
      ...
      if(mParticle.position().x > width/2-260 && mParticle.position().x < width/2-140)
        { gameWon = true; 
          won.play(); 
        }
      
      else if(mParticle.position().x > width/2+240 && mParticle.position().x < width/2+360)
        { gameWon = true; 
          won.play();
        }
...
void mouseClicked() {
  ...
  ball.velocity().set(mouseX-width/2, 3);
  ball.setMaxAge(1000);
  ball.radius(20);
  mPhysics.add(ball);
  clearEdges();
...
void showMonsters(){
  ...
  //rect(width/2+200,height-50,100,50);
  image(monster,width/2-250,height-100,100,100);
  image(monster,width/2+250,height-100,100,100);
}
